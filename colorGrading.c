...
void adjustColorGradingPreset(int sliderValue, int previous) {
    // Calculate image size
    size_t imageSize = copyPresetImage.total() * copyPresetImage.elemSize();

    // Convert the input image to LAB color space
    Mat labImage;
    cvtColor(copyPresetImage, labImage, COLOR_BGR2Lab);
    ...
    // Convert the LAB image back to BGR color space
    cvtColor(labImage, copyPresetImage, COLOR_Lab2BGR);
}
